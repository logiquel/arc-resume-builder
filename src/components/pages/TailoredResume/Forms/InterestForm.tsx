...
interface InterestFormProps {
  onClose: () => void;
  onSave: (data: InterestFormData) => void;
...
const InterestForm: React.FC<InterestFormProps> = ({
  ...
  return (
    <div className="absolute inset-0 w-full h-full flex items-center justify-center z-30 bg-black/20 backdrop-blur-sm">
      <div className="w-96 flex flex-col bg-white border border-black/10 shadow-2xl rounded-3xl ring-4 ring-white/50">
        <header className="w-full flex gap-2 p-3 justify-between shrink-0">
          <div className="flex items-center">
            <Icon icon="solar:gamepad-linear" className="text-brand" />
            <h2 className="text-base font-medium text-text-primary pl-2">
              Add New Interest Entry
            </h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="w-full grid grid-cols-1 gap-x-5 gap-y-4 p-4 overflow-hidden">
